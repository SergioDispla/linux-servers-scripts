...
index(){
sed --in-place "6 c\    <title> $PAG </title>" /var/www/html/$PAG/htdocs/index.html
sed --in-place "193 c\          $PAG" /var/www/html/$PAG/htdocs/index.html
sed --in-place "219 c\          BIENVENIDO A $PAG" /var/www/html/$PAG/htdocs/index.html
}
...
sed --in-place "77 c\Subsystem sftp internal-sftp" /etc/ssh/sshd_config
echo "Match Group usuariosftp
X11Forwarding no
AllowTcpForwarding no
ChrootDirectory %h
ForceCommand internal-sftp" >> /etc/ssh/sshd_config
groupadd usuariosftp
groupmod -g 5000 usuariosftp
if [ $ASK != n ]
then
MK=0
S="s"
	...
	read PAG
	mkdir -p /var/www/html/$PAG/htdocs
	cp /var/www/html/index.html /var/www/html/$PAG/htdocs
	...
	ServerName $PAG
	ServerAlias www.$PAG
	DocumentRoot /var/www/html/$PAG/htdocs
	...
	read SS
		...
		sed --in-place "5 c\		DocumentRoot /var/www/html/$PAG/htdocs" $P
	...
	AU="/etc/apache2/sites-available/$PAG.conf"
		...
		then
		sed --in-place "7 a\  " / $AU
		sed --in-place "8 a\<Directory /var/www/html/$PAG/htdocs/> " $AU
	...
	clear
	echo "Configuracion del servicio WebHosting"
	echo "Su pagina web deben contar con usuario administrador.
Por favor escriba el nombre para el usuario administrador"
	read UADM
	useradd --home-dir /var/www/html/$PAG --no-create-home --shell /bin/false -g usuariosftp $UADM 	
	echo "Digite una contraseña para el usuario $UADM"
	passwd $UADM
	chown $UADM:usuariosftp /var/www/html/$PAG/htdocs
	chown $UADM:usuariosftp /var/www/html/$PAG/htdocs/index.html	
